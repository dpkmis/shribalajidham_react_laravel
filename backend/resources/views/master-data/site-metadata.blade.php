<x-app-layout>
        ...
        <div class="master-card-body">
            ...
            <ul class="nav nav-pills meta-tabs mb-4" role="tablist">
                @foreach($groupLabels as $gKey => $gInfo)
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ $loop->first ? 'active' : '' }}" data-bs-toggle="tab" href="#tab-{{ $gKey }}">
                            <i class="{{ $gInfo['icon'] }} me-1"></i> {{ $gInfo['label'] }}
                        </a>
                    </li>
                    @endif
                @endforeach
            ...
            <form id="metadataForm" enctype="multipart/form-data">
                ...
                <div class="tab-content">
                    @foreach($groupLabels as $gKey => $gInfo)
                        @if(isset($groups[$gKey]))
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $gKey }}">
                            <div class="meta-group-card">
                                <h6><i class="{{ $gInfo['icon'] }} me-2"></i>{{ $gInfo['label'] }}</h6>
                                <div class="row">
                                    @foreach($groups[$gKey] as $meta)
                                    <div class="col-md-{{ in_array($meta->type, ['textarea']) ? '12' : '6' }} meta-field">
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <label>{{ $meta->label }} <small>({{ $meta->key }})</small></label>
                                            <button type="button" class="btn btn-outline-danger delete-field-btn" data-id="{{ $meta->id }}" title="Delete field"><i class="bx bx-trash-alt"></i></button>
                                        </div>

                                        @if($meta->type === 'textarea')
                                            <textarea name="meta[{{ $meta->id }}]" class="form-control" rows="3">{{ $meta->value }}</textarea>
                                        @elseif($meta->type === 'image')
                                            <input type="file" name="meta_file[{{ $meta->id }}]" class="form-control" accept="image/*">
                                            <input type="hidden" name="meta[{{ $meta->id }}]" value="{{ $meta->value }}">
                                            @if($meta->value)
                                                <img src="{{ url($meta->value) }}" class="meta-img-preview" alt="{{ $meta->label }}">
                                            @endif
                                        @elseif($meta->type === 'color')
                                            <input type="color" name="meta[{{ $meta->id }}]" class="form-control form-control-color w-100" value="{{ $meta->value ?? '#000000' }}" style="height: 42px;">
                                        @else
                                            <input type="{{ $meta->type }}" name="meta[{{ $meta->id }}]" class="form-control" value="{{ $meta->value }}"
                                                @if($meta->type === 'tel') placeholder="+91 XXXXX XXXXX" @endif
                                                @if($meta->type === 'email') placeholder="email@example.com" @endif
                                                @if($meta->type === 'url') placeholder="https://" @endif
                                            >
                                        @endif
                                    </div>
                                    @endforeach
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <select id="af_group" class="form-control">
                                @foreach($groupLabels as $gKey => $gInfo)
                                    <option value="{{ $gKey }}">{{ $gInfo['label'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
    ...
    $('#saveAllBtn').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('master.metadata.update') }}",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#addFieldBtn').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('master.metadata.add-field') }}",
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                group: $('#af_group').val(),
    ...
    $(document).on('click', '.delete-field-btn', function() {
        ...
        Swal.fire({
            ...
            $.ajax({
                url: "{{ route('master.metadata.delete-field', ':id') }}".replace(':id', id),
                type: 'POST',
