                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="assignedTo" class="form-control">
                                ...
                                @foreach($staff as $s)
                                    <option value="{{ $s->id }}">{{ $s->full_name }} ({{ ucfirst($s->shift) }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" id="scheduledDate" class="form-control" value="{{ date('Y-m-d') }}">
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                let table = initDataTable({
                    selector: "#tasksTable",
                    ajaxUrl: "{{ route('housekeeping.ajax') }}",
                    moduleName: "Add Task",
                ...
                $('#taskProperty').on('change', function() {
                    ...
                    $.get("{{ route('housekeeping.rooms-by-property') }}", { property_id: propertyId }, function(res) {
                ...
                function updateDashboardStats() {
                    $.get("{{ route('housekeeping.dashboard-stats') }}", function(res) {
        ...
        <script>
            $('#submitTaskBtn').on('click', function () {
                ...
                let url = id
                    ? "{{ route('housekeeping.update', ':id') }}".replace(':id', id)
                    : "{{ route('housekeeping.store') }}";
        ...
        <script>
            $(document).on('click', '.edit-task', function () {
                let id = $(this).data('id');
                let url = "{{ route('housekeeping.show', ':id') }}".replace(':id', id);
        ...
        <script>
            $(document).on('click', '.start-task', function () {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: "{{ route('housekeeping.start-task') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            task_id: id
        ...
        <script>
            $(document).on('click', '.complete-task', function () {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: "{{ route('housekeeping.complete-task') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            task_id: id,
        ...
        <script>
            ...
            $('#submitInspectionBtn').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('housekeeping.inspect-task') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        task_id: taskId,
        ...
        <script>
            $(document).on('click', '.delete-task', function () {
                ...
                Swal.fire({
                    ...
                    let url = "{{ route('housekeeping.destroy', ':id') }}".replace(':id', id);
                    $.ajax({
                        url: url,
                        type: "POST",
                        data: {
