                            ...
                            <div class="role-checkbox-group">
                                <?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="form-check">
                                        <input class="form-check-input user-role" type="checkbox" value="<?php echo e($role->id); ?>" id="role<?php echo e($role->id); ?>">
                                        <label class="form-check-label" for="role<?php echo e($role->id); ?>">
                                            <strong><?php echo e($role->name); ?></strong>
                                            <?php if($role->property): ?>
                                                <span class="badge bg-secondary ms-2"><?php echo e($role->property->name); ?></span>
                                            <?php endif; ?>
                                            <br>
                                            <small class="text-muted"><?php echo e($role->permissions->count()); ?> permissions</small>
                                        </label>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                let table = initDataTable({
                    selector: "#usersTable",
                    ajaxUrl: "<?php echo e(route('users.ajax')); ?>",
                    moduleName: "Add User",
            ...
            function loadUserStats() {
                $.ajax({
                    url: "<?php echo e(route('users.stats')); ?>",
                    type: 'GET',
                    data: {
            ...
            $('#submitUserBtn').on('click', function() {
                ...
                let url = id 
                    ? "<?php echo e(route('users.update', ':id')); ?>".replace(':id', id)
            ...
            $(document).on('click', '.view-user', function() {
                let id = $(this).data('id');
                $.ajax({
                    url: "<?php echo e(route('users.show', ':id')); ?>".replace(':id', id),
                    type: 'GET',
            ...
            function updateUserStatus(id, action) {
                $.ajax({
                    url: "<?php echo e(route('users.status', ':id')); ?>".replace(':id', id).replace('status', action),
                    type: 'POST',
            ...
            $(document).on('click', '.unlock-user', function() {
                let id = $(this).data('id');
                $.ajax({
                    url: "<?php echo e(route('users.unlock', ':id')); ?>".replace(':id', id),
                    type: 'POST',
                    data: { _token: "<?php echo e(csrf_token()); ?>" },
                    success: function(res) {
                        success_noti(res.message);
            ...
            $('#submitResetPasswordBtn').on('click', function() {
                ...
                $.ajax({
                    url: "<?php echo e(route('users.reset-password', ':id')); ?>".replace(':id', currentUserId),
                    type: 'POST',
                    data: {
                        _token: "<?php echo e(csrf_token()); ?>",
                        password: password,
            ...
            $(document).on('click', '.view-activity', function() {
                let id = $(this).data('id');
                $.ajax({
                    url: "<?php echo e(route('users.activity', ':id')); ?>".replace(':id', id),
                    type: 'GET',
                    success: function(data) {
...
<?php endif; ?><?php /**PATH /var/www/html/shribalajidham_react_laravel/backend/resources/views/users/index.blade.php ENDPATH**/ ?>